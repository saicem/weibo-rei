        ...
        def init_url_by_user_id_and_date():
            ...
            while start_date < end_date:
                for user_id in user_ids:
                    ...
                    urls.append(
                        url_format.format(user_id, start_date_string, end_date_string)
                    )
        ...
        if response.url.endswith("page=1"):
            ...
            if all_page:
                ...
                for page_num in range(2, all_page + 1):
                    ...
                    yield Request(
                        page_url, self.parse, dont_filter=True, meta=response.meta
                    )
                # 如果是搜索接口，按照天的粒度结果已经是100页，那继续按照小时的粒度进行切分
                if (
                    "search/mblog" in response.url
                    and all_page == 100
                    and "-" not in response.url
                ):
                    start_time_string = re.search(
                        r"starttime=(\d+)&", unquote(response.url, "utf-8")
                    ).group(1)
                    keyword = re.search(
                        r"keyword=(.*?)&", unquote(response.url, "utf-8")
                    ).group(1)
                    self.logger.info(
                    ...
                    for hour in range(7, 24):
                        ...
                        begin_hour = (
                            one_day_back.strftime("%Y%m%d") + "-" + str(hour + 16)
                        )
                    ...
                    for hour in range(0, 7):
                        ...
                        begin_hour = (
                            two_day_back.strftime("%Y%m%d") + "-" + str(hour + 40)
                        )
                        end_hour = (
                            two_day_back.strftime("%Y%m%d") + "-" + str(hour + 17)
                        )
                        page_url = url_format_by_hour.format(
                            keyword, begin_hour, end_hour
                        )
