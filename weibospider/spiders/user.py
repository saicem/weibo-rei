...
class UserSpider(Spider):
    ...
    def parse(self, response):
        ...
        if nick_name and nick_name[0]:
            user_item["nick_name"] = nick_name[0].replace("\xa0", "")
        if gender and gender[0]:
            user_item["gender"] = gender[0].replace("\xa0", "")
        if place and place[0]:
            place = place[0].replace("\xa0", "").split(" ")
            user_item["province"] = place[0]
            if len(place) > 1:
                user_item["city"] = place[1]
        if brief_introduction and brief_introduction[0]:
            user_item["brief_introduction"] = brief_introduction[0].replace("\xa0", "")
        if birthday and birthday[0]:
            user_item["birthday"] = birthday[0]
        if sex_orientation and sex_orientation[0]:
            if sex_orientation[0].replace("\xa0", "") == gender[0]:
                user_item["sex_orientation"] = "同性恋"
            else:
                user_item["sex_orientation"] = "异性恋"
        if sentiment and sentiment[0]:
            user_item["sentiment"] = sentiment[0].replace("\xa0", "")
        if vip_level and vip_level[0]:
            user_item["vip_level"] = vip_level[0].replace("\xa0", "")
        if authentication and authentication[0]:
            user_item["authentication"] = authentication[0].replace("\xa0", "")
        if labels and labels[0]:
            user_item["labels"] = (
                labels[0].replace("\xa0", ",").replace(";", "").strip(",")
            )
        education_info = (
            selector.xpath('//div[contains(text(),"学习经历")]/following-sibling::div[1]')
            .xpath("string(.)")
            .extract()
        )
        if education_info:
            user_item["education"] = education_info[0].replace("\xa0", "")
        work_info = (
            selector.xpath('//div[contains(text(),"工作经历")]/following-sibling::div[1]')
            .xpath("string(.)")
            .extract()
        )
        if work_info:
            user_item["work"] = work_info[0].replace("\xa0", "")
        request_meta = response.meta
        request_meta["item"] = user_item
        yield Request(
            self.base_url + "/u/{}".format(user_item["_id"]),
