...
class CommentSpider(Spider):
    ...
    def start_requests(self):
        ...
        urls = [
            f"{self.base_url}/comment/hot/{tweet_id}?rl=1&page=1"
    ...
    def parse(self, response):
        ...
        for comment_node in comment_nodes:
            ...
            comment_item["comment_user_id"] = re.search(
                r"/u/(\d+)", comment_user_url[0]
            ).group(1)
            comment_item["content"] = extract_comment_content(
                etree.tostring(comment_node, encoding="unicode")
            )
